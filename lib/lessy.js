module.exports = (function(){
	...
	return function(output,imports){
			...
			app.log = logger(function(){ return app.verbose; });
			...
			app.generateParser = function Parser(force){
				if(this.parser && !force) return this.parser;
				//clear for garbage and add up a new parser
				delete this.parser;
			...
			app.sync = function Sync(set,endpoint){
				...
				point = (!set.match(/^\/|^\.\//)) ? path.resolve(imports,set) : set;
				...
				return this.watch(endpoint,point,function(n){
					if(!fs.existsSync(this.root)) return false;
					var rt = this.root,
						data = readFile(this.root),
						out = path.resolve(self.out,endpoint);
					return self.parser.parse(data,function(e,tree){
						if(e){ 
							self.log.crisis = 1; self.log.error(e.message ? e.message : e);  self.log.crisis = 0;
							return self.generateParser(true);
						}
						self.log.log("   Sync'ing: {0}".grey,n.green);
			...
			app.syncDir = function Sync(sets){
				...
				util.eachAsync(lists,function(e,i,o,fn){
					if(!e.match(/\.less$/) || e.match(/^\_\w+/)) return;
			...
			var oldbt = app.bootup;

			app.bootup = function(){
				this.log.crisis = 1;
				this.log.log('Booting '.grey+'{0}' + ' with a '.grey + '{1} ' +'timelapse!'.grey,'Lessy'.green,(this.ms.toString()+"ms").magenta);
				this.log.crisis = 0;
				this.verbose = true;
				oldbt.call(this);
			};
